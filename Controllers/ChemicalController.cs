...
namespace FIFO_Infineon.Controllers
{
    public class ChemicalController : Controller
    {
        ...
        public IActionResult Out()
        {
            ...
            ViewData["MasterItemID"] = new SelectList(chemicalItems, "ItemID", "ItemName");
        ...
        public async Task<IActionResult> Out([Bind("MasterItemID,Quantity")] StockItem model)
        {
            ...
            if (!ModelState.IsValid)
            {
                ...
                var chemicalItems = await _context.MasterItems
                    .Where(m => m.Category == "Chemical")
                    .OrderBy(m => m.ItemName)
                    .ToListAsync();
                ViewData["MasterItemID"] = new SelectList(chemicalItems, "ItemID", "ItemName", model.MasterItemID);
                return View(model);
            }
