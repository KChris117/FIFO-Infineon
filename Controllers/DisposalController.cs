...
namespace FIFO_Infineon.Controllers
{
    public class DisposalController : Controller
    {
        ...
        public DisposalController(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> DisposalStockItem()
        {
            var stockList = await _context.StockItems
                .Where(s => s.MasterItem != null && s.MasterItem.Category == "Disposal")
        ...
        {
            var disposalItems = _context.MasterItems
                .Where(m => m.Category == "Disposal")
                .OrderBy(m => m.ItemID)
                .ToList();

            // Properti kedua dan ketiga di SelectList harus sama
            ViewData["MasterItemID"] = new SelectList(disposalItems, "ItemID", "ItemID");
        ...
        public IActionResult Out()
        {
            ...
            ViewData["MasterItemID"] = new SelectList(disposalItems, "ItemID", "ItemName");
        ...
        public async Task<IActionResult> Out([Bind("MasterItemID,Quantity")] StockItem model)
        {
            ...
            if (!ModelState.IsValid)
            {
                ...
                var disposalItems = await _context.MasterItems
                    .Where(m => m.Category == "Disposal")
                    .OrderBy(m => m.ItemName)
                    .ToListAsync();
                ViewData["MasterItemID"] = new SelectList(disposalItems, "ItemID", "ItemName", model.MasterItemID);
                return View(model);
            }
